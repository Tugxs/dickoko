...
export function createDiscordRequestGate({ request = fetch, requestsPerSecond = 8, concurrency = 2, globalRequestsPerSecond = 20, globalConcurrency = 4, maxPending = 500, maxTotalPending = 600, maxWaitMs = 30_000, invalidRequestLimit = 50, invalidBotLimit = 12, invalidWindowMs = 600_000, invalidCooldownMs = 300_000 } = {}) {
  const buckets = new Map();
  const intervalMs = Math.ceil(1_000 / requestsPerSecond);
  const globalIntervalMs = Math.ceil(1_000 / globalRequestsPerSecond);
  const global = { active: 0, nextAt: 0, pending: 0, invalidAt: [], invalidUntil: 0 };
  let lastSweep = 0;
  ...
  function pump(bucket) {
    ...
    while (bucket.active < concurrency && global.active < globalConcurrency && bucket.jobs.length) {
      ...
      for (let index = 0; index < bucket.jobs.length; index++) {
        ...
        if (global.invalidUntil > now) {
          bucket.jobs.splice(index--, 1); global.pending--;
          job.resolve(unavailable(503, global.invalidUntil - now, 'أوقفنا طلبات Discord مؤقتًا لحماية اتصال الخدمة. حاول بعد انتهاء المهلة.'));
          continue;
        }
        ...
        if (bucket.invalidBotUntil > now) {
          bucket.jobs.splice(index--, 1); global.pending--;
          job.resolve(unavailable(403, bucket.invalidBotUntil - now, 'أوقفنا طلبات هذا البوت مؤقتًا بعد أخطاء متكررة. راجع صلاحياته وبيانات القالب.'));
          continue;
        }
      ...
      void (async () => {
        try {
          ...
          if ([401, 403, 429].includes(response.status) && response.headers.get('x-ratelimit-scope') !== 'shared') {
            const now = Date.now();
            bucket.invalidAt = bucket.invalidAt.filter(at => now - at < invalidWindowMs);
            bucket.invalidAt.push(now);
            if (bucket.invalidAt.length >= invalidBotLimit) bucket.invalidBotUntil = Math.max(bucket.invalidBotUntil, now + invalidCooldownMs);
            global.invalidAt = global.invalidAt.filter(at => now - at < invalidWindowMs);
            global.invalidAt.push(now);
            if (global.invalidAt.length >= invalidRequestLimit) {
              global.invalidUntil = Math.max(global.invalidUntil, now + invalidCooldownMs);
              console.error('Discord outbound protection paused requests', { invalidRequests: global.invalidAt.length, cooldownMs: invalidCooldownMs });
            }
          }
  ...
  return (key, route, url, options) => {
    const now = Date.now();
    if (global.invalidUntil > now) return Promise.resolve(unavailable(503, global.invalidUntil - now, 'أوقفنا طلبات Discord مؤقتًا لحماية اتصال الخدمة. حاول بعد انتهاء المهلة.'));
    ...
    if (!bucket) {
      bucket = { jobs: [], active: 0, nextAt: 0, globalUntil: 0, authUntil: 0, invalidAt: [], invalidBotUntil: 0, routes: new Map(), invalidCounts: new Map(), invalidUntil: new Map(), timer: null, lastUsed: now };
      buckets.set(key, bucket);
    }
    bucket.lastUsed = now;
    if (bucket.invalidBotUntil > now) return Promise.resolve(unavailable(403, bucket.invalidBotUntil - now, 'أوقفنا طلبات هذا البوت مؤقتًا بعد أخطاء متكررة. راجع صلاحياته وبيانات القالب.'));
