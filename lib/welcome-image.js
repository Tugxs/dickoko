...
export async function validateWelcomeGif(input) {
  ...
  const metadata = await sharp(bytes, { animated: true, limitInputPixels: 100_000_000 }).metadata();
  const frames = metadata.pages || 1;
  const frameHeight = metadata.pageHeight || metadata.height;
  if (!metadata.width || !frameHeight || frames > 100 || metadata.width * frameHeight * frames > 60_000_000) {
    throw Error('تصميم GIF كبير جدًا للدمج. استخدم GIF أقصر أو أصغر أبعادًا.');
  }
  return metadata;
}
