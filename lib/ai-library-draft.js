...
export function libraryDraftProposal({ mode, category, title, prompt }) {
  ...
  else if (template.kind === 'rules') interactive = { kind: 'rules', title: namedTitle || '📜 قوانين السيرفر', description: 'أهلًا بك! اقرأ القوانين التالية لتحافظ على بيئة ممتعة وآمنة للجميع.', rules: [...defaultServerRules], style: 'single', channel };
