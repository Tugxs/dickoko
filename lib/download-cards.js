...
export function mountDownloadCards(app, { pool, requireUser, requireWriteAccess, authorizedGuild, discordBotFetch, baseUrl }) {
  app.post('/api/ai/requests/:id/launch-download', requireUser, requireWriteAccess, async (req, res, next) => {
    ...
    try {
      ...
      if (!title || !description) throw Object.assign(new Error('اكتب عنوان البطاقة ووصفها.'), { status: 400 });
      ...
      const sent = await discordBotFetch(`/channels/${publication.id}/messages`, discordMessageOptions(card, item.attachment));
      ...
      await client.query('UPDATE ai_requests SET interactive_message_id=$1,interactive_channel_id=$2,interactive_kind=$3 WHERE id=$4', [sent.data.id, publication.id, 'download', item.id]);
