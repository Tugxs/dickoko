...
export function normalizeAiProposal(input) {
  ...
  for (const item of Array.isArray(input.operations) ? input.operations.slice(0, 8) : []) {
    ...
    operations.push({ resource_type: item.resource_type, action: 'create', name, ...(item.resource_type === 'channel' ? { type: [0, 2, 15].includes(Number(item.type)) ? Number(item.type) : 0 } : {}) });
  }
  let message = null;
  ...
  return operations.length || message ? { operations, message } : null;
}
...
export function mountLocalAi(app, { pool, requireUser, requireWriteAccess, authorizedGuild, canonicalPlan, discordBotFetch }) {
  ...
  app.get('/api/ai/conversations/:id/messages', requireUser, async (req, res, next) => { try {
    ...
    const { rows } = await pool.query('SELECT id,prompt,answer,status,error,proposal,sent_message_id,sent_channel_id,has_attachment,created_at,completed_at FROM (SELECT id,prompt,answer,status,error,proposal,sent_message_id,sent_channel_id,(attachment IS NOT NULL) AS has_attachment,created_at,completed_at FROM ai_requests WHERE conversation_id=$1 AND user_id=$2 ORDER BY created_at DESC LIMIT 100) recent ORDER BY created_at', [conversation.id, req.user.id]);
  ...
  app.post('/api/ai/requests', requireUser, requireWriteAccess, async (req, res, next) => { try {
    ...
    try {
      ...
      if (conversationId) {
        ...
        if (!attachment && prompt.length < 80 && /(نفذ|نفّذ|طبق|طبّق|يلا|موافق|أوافق|نعم|أيوه)/i.test(prompt)) {
          const previousImage = (await client.query("SELECT attachment FROM ai_requests WHERE conversation_id=$1 AND user_id=$2 AND attachment IS NOT NULL AND created_at > NOW()-INTERVAL '2 hours' ORDER BY created_at DESC LIMIT 1", [conversationId, req.user.id])).rows[0];
          attachment = previousImage?.attachment || null;
        }
      } else {
  ...
  app.post('/api/ai/requests/:id/send-message', requireUser, requireWriteAccess, async (req, res, next) => { try {
    ...
    try {
      await client.query('BEGIN');
      const item = (await client.query('SELECT id,guild_id,proposal,attachment,sent_message_id,sent_channel_id FROM ai_requests WHERE id=$1 AND user_id=$2 FOR UPDATE', [req.params.id, req.user.id])).rows[0];
      if (!item || !item.proposal?.message) { await client.query('ROLLBACK'); return res.status(404).json({ error: 'لا توجد رسالة جاهزة للإرسال' }); }
      ...
      if (!channel) { await client.query('ROLLBACK'); return res.status(409).json({ error: `قناة ${item.proposal.message.channel} غير موجودة أو ليست نصية` }); }
      ...
      if (image) {
        ...
        form.append('files[0]', new Blob([bytes], { type: mime }), `diskoko-image.${mime.split('/')[1] === 'jpeg' ? 'jpg' : mime.split('/')[1]}`);
  ...
  app.get('/api/ai/worker/next', worker, async (req, res, next) => { try {
    ...
    if (discordBotFetch) {
      ...
      guildContext = { name: remote.ok ? String(remote.data.name || '').slice(0, 100) : '', channels: channels.ok && Array.isArray(channels.data) ? channels.data.map(item => ({ id: item.id, name: item.name, type: item.type })).slice(0, 100) : [], roles: roles.ok && Array.isArray(roles.data) ? roles.data.map(item => ({ name: item.name })).slice(0, 100) : [] };
    }
  ...
  app.post('/api/ai/worker/:id/complete', worker, async (req, res, next) => { try {
    ...
    const proposal = error ? null : normalizeAiProposal(req.body.proposal);
    const { rowCount } = await pool.query("UPDATE ai_requests SET status=$1,answer=$2,error=$3,proposal=$4,completed_at=NOW() WHERE id=$5 AND status='processing'", [error ? 'failed' : 'completed', error ? null : answer, error || null, proposal, req.params.id]);
