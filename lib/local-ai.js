...
export function presentAiRequest(item) {
  ...
  return { ...item, can_publish_answer: false, can_select_step: planning, ...(planning ? { proposal: null } : item.library_mode === 'execute' ? {} : item.proposal?.interactive ? { proposal: alignAiProposalWithIntent({ ...item.proposal, executeNow: true }, [{ role: 'user', content: item.proposal.review_request || item.prompt }]) } : {}) };
}
