...
export function mountLocalAi(app, { pool, requireUser, requireWriteAccess, authorizedGuild, canonicalPlan, discordBotFetch }) {
  ...
  app.get('/api/ai/conversations/:id/messages', requireUser, async (req, res, next) => { try {
    ...
    const { rows } = await pool.query('SELECT recent.*,(SELECT status FROM change_sets WHERE id=recent.change_set_id) AS change_set_status FROM (SELECT id,prompt,answer,status,error,proposal,sent_message_id,sent_channel_id,interactive_message_id,interactive_channel_id,interactive_kind,change_set_id,(attachment IS NOT NULL) AS has_attachment,created_at,completed_at FROM ai_requests WHERE conversation_id=$1 AND user_id=$2 ORDER BY created_at DESC LIMIT 100) recent ORDER BY created_at', [conversation.id, req.user.id]);
    const messages = rows.map(item => item.proposal?.interactive ? { ...item, proposal: alignAiProposalWithIntent({ ...item.proposal, executeNow: true }, [{ role: 'user', content: item.proposal.review_request || item.prompt }]) } : item);
  ...
  app.get('/api/ai/worker/next', worker, async (req, res, next) => { try {
    ...
    RETURNING id,user_id,guild_id,prompt,conversation_id,attachment,(attachment IS NOT NULL) AS has_attachment`);
    const job = rows[0];
    ...
    res.json({ request: { id: job.id, guild_id: job.guild_id, prompt: job.prompt, has_attachment: job.has_attachment, image: job.attachment || null, context, guild_context: guildContext } });
  } catch (error) { next(error); } });
  ...
  app.post('/api/ai/worker/:id/complete', worker, async (req, res, next) => { try {
    ...
    if (!error && (submitted?.message || submitted?.interactive)) {
