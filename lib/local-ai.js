  ...
  app.post('/api/ai/requests/:id/send-message', requireUser, requireWriteAccess, async (req, res, next) => { try {
    ...
    try {
      ...
      if (image) {
        ...
        form.append('payload_json', JSON.stringify(video ? { content, allowed_mentions: { parse: [] } } : { embeds: req.body.imagePosition === 'below' ? [textEmbed, imageEmbed] : [imageEmbed, textEmbed], allowed_mentions: { parse: [] } }));
