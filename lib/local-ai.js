...
export function presentAiRequest(item) {
  ...
  const interactive = ['giveaway', 'tickets', 'poll', 'event', 'welcome', 'scheduled_event', 'rules'].includes(candidate?.interactive?.kind) ? candidate.interactive : null;
...
export function mountLocalAi(app, { pool, requireUser, requireWriteAccess, authorizedGuild, canonicalPlan, discordBotFetch, requirePlanCapacity }) {
  ...
  app.post('/api/ai/requests/:id/send-message', requireUser, requireWriteAccess, async (req, res, next) => { try {
    ...
    try {
      ...
      await client.query('UPDATE ai_requests SET sent_message_id=$1,sent_channel_id=$2,published_at=NOW(),publishing_bot_id=$4 WHERE id=$3', [sent.data.id, channel.id, item.id, req.publishingBotId || null]);
      await client.query('COMMIT');
