...
import { libraryDraftProposal } from './ai-library-draft.js';
...
export function normalizeAiProposal(input) {
  ...
  for (const item of Array.isArray(input.operations) ? input.operations.slice(0, 30) : []) {
    ...
    if (!name || /[\r\n@]/.test(name) || /\[[^\]]+\]/.test(name) || (item.resource_type === 'role' && /^(admin|administrator|owner|مدير|مالك)$/i.test(name))) continue;
...
export function mountLocalAi(app, { pool, requireUser, requireWriteAccess, authorizedGuild, canonicalPlan, discordBotFetch, requirePlanCapacity }) {
  ...
  app.get('/api/ai/conversations/:id/messages', requireUser, async (req, res, next) => { try {
    ...
    const messages = rows.map(item => ({ ...item, can_publish_answer: !item.library_mode && !planningRequest(item.prompt), can_select_step: item.library_mode === 'advice' || planningRequest(item.prompt), ...(item.library_mode === 'advice' || planningRequest(item.prompt) ? { proposal: null } : item.proposal?.interactive ? { proposal: alignAiProposalWithIntent({ ...item.proposal, executeNow: true }, [{ role: 'user', content: item.proposal.review_request || item.prompt }]) } : {}) }));
  ...
  app.post('/api/ai/requests/:id/send-message', requireUser, requireWriteAccess, async (req, res, next) => { try {
    ...
    try {
      ...
      if (item?.library_mode && req.body.publishAnswer === true) { await client.query('ROLLBACK'); return res.status(409).json({ error: 'نص المكتبة ليس رسالة للنشر. استخدم بطاقة الإجراء المخصصة للمراجعة والتنفيذ.' }); }
