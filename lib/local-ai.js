...
export function normalizeAiProposal(input) {
  ...
  const operations = [];
  for (const item of Array.isArray(input.operations) ? input.operations.slice(0, 30) : []) {
    ...
    const parentName = item.resource_type === 'channel' && item.action !== 'update' ? String(item.parent_name || '').trim().slice(0, 100) : '';
    if (operations.some(op => op.resource_type === item.resource_type && op.name.toLowerCase() === name.toLowerCase() && (op.parent_name || '').toLowerCase() === parentName.toLowerCase())) continue;
    const updating = item.action === 'update';
    const resourceId = String(item.resource_id || '');
    if (updating && !/^\d{15,22}$/.test(resourceId)) continue;
    const operation = { resource_type: item.resource_type, action: updating ? 'update' : 'create', name, ...(updating ? { resource_id: resourceId } : item.resource_type === 'channel' ? { type: [0, 2, 15].includes(Number(item.type)) ? Number(item.type) : 0, ...(parentName ? { parent_name: parentName } : {}) } : {}) };
    if (!updating && item.resource_type === 'channel' && ['read_only', 'staff_only'].includes(item.access)) {
      operation.access = item.access;
      if (item.access === 'staff_only' && /^\d{15,22}$/.test(String(item.staff_role_id || ''))) operation.staff_role_id = String(item.staff_role_id);
      else if (item.access === 'staff_only') continue;
    }
  ...
  } else if (input.interactive?.kind === 'download') {
    const title = String(input.interactive.title || '').trim().slice(0, 100);
    const description = String(input.interactive.description || '').trim().slice(0, 800);
    const channel = String(input.interactive.channel || '').trim().replace(/^#/, '').slice(0, 100);
    if (title && description && channel) interactive = { kind: 'download', title, description, channel };
  }
...
export function mountLocalAi(app, { pool, requireUser, requireWriteAccess, authorizedGuild, canonicalPlan, discordBotFetch, requirePlanCapacity }) {
  ...
  app.get('/api/ai/conversations/:id/messages', requireUser, async (req, res, next) => { try {
    ...
    const { rows } = await pool.query('SELECT recent.*,(SELECT status FROM change_sets WHERE id=recent.change_set_id) AS change_set_status FROM (SELECT id,prompt,answer,status,error,proposal,library_mode,library_title,sent_message_id,sent_channel_id,interactive_message_id,interactive_channel_id,interactive_kind,change_set_id,(attachment IS NOT NULL) AS has_attachment,created_at,completed_at FROM ai_requests WHERE conversation_id=$1 AND user_id=$2 ORDER BY created_at DESC LIMIT 100) recent ORDER BY created_at', [conversation.id, req.user.id]);
    const messages = rows.map(item => ({ ...item, can_publish_answer: item.library_mode !== 'advice' && !planningRequest(item.prompt), can_select_step: item.library_mode === 'advice' || planningRequest(item.prompt), ...(item.library_mode === 'advice' || planningRequest(item.prompt) ? { proposal: null } : item.proposal?.interactive ? { proposal: alignAiProposalWithIntent({ ...item.proposal, executeNow: true }, [{ role: 'user', content: item.proposal.review_request || item.prompt }]) } : {}) }));
  ...
  app.post('/api/ai/requests', requireUser, requireWriteAccess, async (req, res, next) => { try {
    ...
    try {
      ...
      await client.query('INSERT INTO ai_requests(id,user_id,guild_id,prompt,conversation_id,attachment,library_mode,library_title) VALUES($1,$2,$3,$4,$5,$6,$7,$8)', [id, req.user.id, guildId, prompt, conversationId, attachment, libraryMode, libraryTitle]);
  ...
  app.post('/api/ai/requests/:id/send-message', requireUser, requireWriteAccess, async (req, res, next) => { try {
    ...
    try {
      await client.query('BEGIN');
      const item = (await client.query('SELECT id,guild_id,prompt,library_mode,status,answer,proposal,attachment,sent_message_id,sent_channel_id FROM ai_requests WHERE id=$1 AND user_id=$2 FOR UPDATE', [req.params.id, req.user.id])).rows[0];
      if (item && (item.library_mode === 'advice' || planningRequest(item.prompt))) { await client.query('ROLLBACK'); return res.status(409).json({ error: 'هذه أفكار أو خطة عمل، وليست رسالة للنشر. اختر فكرة أو تغييرًا محددًا لتطبيقه.' }); }
      ...
      if (image) {
        ...
        form.append('payload_json', JSON.stringify({ embeds: req.body.imagePosition === 'below' ? [textEmbed, imageEmbed] : [imageEmbed, textEmbed], allowed_mentions: { parse: [] } }));
  ...
  app.get('/api/ai/worker/next', worker, async (req, res, next) => { try {
    ...
    RETURNING id,user_id,guild_id,prompt,library_mode,library_title,conversation_id,attachment,(attachment IS NOT NULL) AS has_attachment`);
    const job = rows[0];
    ...
    res.json({ request: { id: job.id, guild_id: job.guild_id, prompt: job.prompt, library_mode: job.library_mode, library_title: job.library_title, has_attachment: job.has_attachment, image: job.attachment || null, context, guild_context: guildContext } });
  } catch (error) { next(error); } });
  ...
  app.post('/api/ai/worker/:id/complete', worker, async (req, res, next) => { try {
    ...
    const libraryMode = (await pool.query('SELECT library_mode FROM ai_requests WHERE id=$1 AND status=$2', [req.params.id, 'processing'])).rows[0]?.library_mode;
    const proposal = error || libraryMode === 'advice' ? null : normalizeAiProposal(submitted);
