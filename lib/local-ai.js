...
export function mountLocalAi(app, { pool, requireUser, requireWriteAccess, authorizedGuild, canonicalPlan, discordBotFetch }) {
  ...
  app.get('/api/ai/conversations/:id/messages', requireUser, async (req, res, next) => { try {
    ...
    const { rows } = await pool.query('SELECT id,prompt,answer,status,error,proposal,sent_message_id,sent_channel_id,created_at,completed_at FROM (SELECT id,prompt,answer,status,error,proposal,sent_message_id,sent_channel_id,created_at,completed_at FROM ai_requests WHERE conversation_id=$1 AND user_id=$2 ORDER BY created_at DESC LIMIT 100) recent ORDER BY created_at', [conversation.id, req.user.id]);
  ...
  app.post('/api/ai/requests/:id/send-message', requireUser, requireWriteAccess, async (req, res, next) => { try {
    ...
    try {
      await client.query('BEGIN');
      const item = (await client.query('SELECT id,guild_id,proposal,sent_message_id,sent_channel_id FROM ai_requests WHERE id=$1 AND user_id=$2 FOR UPDATE', [req.params.id, req.user.id])).rows[0];
  ...
  app.get('/api/ai/worker/next', worker, async (req, res, next) => { try {
    ...
    res.json({ request: { id: job.id, guild_id: job.guild_id, prompt: job.prompt, context, guild_context: guildContext } });
  } catch (error) { next(error); } });
