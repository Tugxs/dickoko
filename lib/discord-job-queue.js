...
import { createDiscordRequestGate, discordRetryAfterMs } from './discord-rate-limit.js';
...
export async function migrateDiscordJobQueue(pool) {
  ...
  ALTER TABLE discord_job_lanes ADD COLUMN IF NOT EXISTS cooldown_until TIMESTAMPTZ;
  CREATE TABLE IF NOT EXISTS discord_job_route_limits (
    bot_key TEXT NOT NULL REFERENCES discord_job_lanes(bot_key), route TEXT NOT NULL,
    cooldown_until TIMESTAMPTZ NOT NULL, PRIMARY KEY(bot_key,route)
  );
  CREATE TABLE IF NOT EXISTS discord_jobs (
    ...
    authorization_cipher TEXT NOT NULL, headers JSONB NOT NULL DEFAULT '{}'::jsonb,
...
export async function enqueueDiscordJob(pool, { botKey, route, pathname, authorization, options = {} }) {
  ...
  const cooldown = (await pool.query(`SELECT GREATEST(COALESCE(l.cooldown_until,'1970-01-01'::timestamptz),COALESCE(r.cooldown_until,'1970-01-01'::timestamptz)) AS until
    FROM discord_job_lanes l LEFT JOIN discord_job_route_limits r ON r.bot_key=l.bot_key AND r.route=$2 WHERE l.bot_key=$1`, [botKey, route])).rows[0]?.until;
  const remaining = new Date(cooldown || 0).getTime() - Date.now();
  if (remaining > 50_000) return { ok: false, status: 429, data: { message: 'Discord طلب الانتظار لهذا البوت أو المسار. أعد المحاولة بعد انتهاء المهلة.' }, headers: new Headers({ 'retry-after': String(Math.ceil(remaining / 1_000)) }) };
  await pool.query('INSERT INTO discord_jobs(id,bot_key,route,pathname,method,authorization_cipher,headers,body) VALUES($1,$2,$3,$4,$5,$6,$7,$8)',
...
export async function claimDiscordJob(pool) {
  ...
  try {
    ...
    const row = (await client.query(`SELECT j.* FROM discord_jobs j JOIN discord_job_lanes l ON l.bot_key=j.bot_key
      LEFT JOIN discord_job_route_limits r ON r.bot_key=j.bot_key AND r.route=j.route
      ...
      AND (l.cooldown_until IS NULL OR l.cooldown_until<=NOW()) AND (r.cooldown_until IS NULL OR r.cooldown_until<=NOW())
...
export async function executeDiscordJob(pool, job) {
  try {
    ...
    const headers = { ...job.headers, Authorization: open(job.authorization_cipher) };
    if (body instanceof FormData) { delete headers['Content-Type']; delete headers['content-type']; }
    else if (typeof body === 'string' && !Object.keys(headers).some(name => name.toLowerCase() === 'content-type')) headers['Content-Type'] = 'application/json';
    ...
    if (response.status === 429) {
      ...
      const delay = discordRetryAfterMs(payload, response.headers);
      if (payload.global === true || response.headers.get('x-ratelimit-scope') === 'global') {
        await pool.query("UPDATE discord_job_lanes SET cooldown_until=GREATEST(COALESCE(cooldown_until,NOW()),NOW()+($2::bigint * INTERVAL '1 millisecond')) WHERE bot_key=$1", [job.bot_key, delay]);
      } else {
        await pool.query("INSERT INTO discord_job_route_limits(bot_key,route,cooldown_until) VALUES($1,$2,NOW()+($3::bigint * INTERVAL '1 millisecond')) ON CONFLICT(bot_key,route) DO UPDATE SET cooldown_until=GREATEST(discord_job_route_limits.cooldown_until,EXCLUDED.cooldown_until)", [job.bot_key, job.route, delay]);
      }
    } else if (response.headers.get('x-ratelimit-remaining') === '0') {
      const delay = Math.ceil(Number(response.headers.get('x-ratelimit-reset-after')) * 1_000);
      if (Number.isFinite(delay) && delay > 0) await pool.query("INSERT INTO discord_job_route_limits(bot_key,route,cooldown_until) VALUES($1,$2,NOW()+($3::bigint * INTERVAL '1 millisecond')) ON CONFLICT(bot_key,route) DO UPDATE SET cooldown_until=GREATEST(discord_job_route_limits.cooldown_until,EXCLUDED.cooldown_until)", [job.bot_key, job.route, delay]);
    }
...
export async function recoverDiscordJobQueue(pool) {
  ...
  await pool.query("DELETE FROM discord_job_route_limits WHERE cooldown_until < NOW() - INTERVAL '1 hour'");
}
