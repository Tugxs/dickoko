...
export function mountWorkspace(app, deps) {
  ...
  app.get('/api/workspace/:guildId', requireUser, route(async (req, res) => {
    ...
    const [remote, channels, roles, changes, events, preferences, draft, publications, failedSchedules, pausedGiveaways] = await Promise.all([
    ...
    const alerts = [
      ...(!botStatus().online ? [{ kind: 'bot', title: 'البوت غير متصل', detail: 'تحقق من تشغيل البوت وربطه قبل تنفيذ التغييرات.', target: 'settings' }] : []),
      ...(status !== 'installed' || !readable ? [{ kind: 'connection', title: 'اتصال السيرفر يحتاج مراجعة', detail: status === 'unavailable' ? 'تعذر التحقق من Discord مؤقتًا؛ لم نغيّر حالة الربط.' : 'راجع ربط البوت وصلاحيات قراءة السيرفر.', target: 'settings' }] : []),
      ...pausedGiveaways.rows.map(row => ({ id: row.id, kind: 'giveaway', title: `توقف إعلان الجيف أوي: ${row.prize}`, detail: row.last_error === 'Discord 404' ? 'رسالة الجيف أوي الأصلية غير متاحة. أوقفنا المحاولات تلقائيًا؛ أنشئ جيف أوي جديدًا إذا رغبت.' : `أوقفنا المحاولات بعد ${row.failure_count} إخفاقات. يمكنك طلب محاولة واحدة بعد مراجعة القناة.`, channelId: row.channel_id, retryable: row.last_error !== 'Discord 404', createdAt: row.ends_at })),
      ...failedSchedules.rows.map(row => ({ id: row.id, kind: 'schedule', title: 'توقفت رسالة مجدولة', detail: row.last_error || 'راجع القناة والصلاحيات ثم أنشئ المهمة من جديد.', target: 'automation', createdAt: row.updated_at })),
    ];
    res.json({ guild: { ...guild, name: remote.ok ? remote.data.name : guild.name }, connection: { status, checked_at: new Date().toISOString(), readable },
      ...
      changeSets: changes.rows, publications: publications.rows, activity: events.rows, alerts, draft: draft.rows[0] || null,
  ...
  app.post('/api/workspace/:guildId/alerts/:kind/:id/dismiss', requireUser, requireWriteAccess, route(async (req, res) => {
    ...
    if (!['giveaway', 'schedule'].includes(kind)) throw problem('نوع التنبيه غير صالح.', 400);
    const table = kind === 'giveaway' ? 'diskoko_giveaways' : 'scheduled_messages';
    const owned = kind === 'giveaway'
      ? await pool.query(`SELECT id FROM ${table} WHERE id=$1 AND guild_id=$2 AND status='paused'`, [req.params.id, guild.id])
      : await pool.query(`SELECT id FROM ${table} WHERE id=$1 AND guild_id=$2 AND user_id=$3 AND status='failed'`, [req.params.id, guild.id, req.user.id]);
    if (!owned.rowCount) throw problem('هذا التنبيه لم يعد متاحًا.', 404);
