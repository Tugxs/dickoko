...
export function mountWorkspace(app, deps) {
  ...
  app.post('/api/change-sets', requireUser, requireWriteAccess, route(async (req, res) => {
    ...
    try {
      ...
      if (aiRequestId) {
        const aiRequest = (await client.query("SELECT id,guild_id,prompt,proposal,change_set_id FROM ai_requests WHERE id=$1 AND user_id=$2 AND status='completed' FOR UPDATE", [aiRequestId, req.user.id])).rows[0];
