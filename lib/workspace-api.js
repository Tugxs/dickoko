import { problem, normalizeOperations, operationBody, resolveExisting, checkConflict, connectionState, normalizeSchedule } from './workspace-domain.js';
...
export function mountWorkspace(app, deps) {
  const { pool, requireUser, authorizedGuild, discordBotFetch: discord, audit, requirePlanCapacity, templates, makeTemplatePlan, botStatus } = deps;
  ...
  app.get('/api/workspace/:guildId', requireUser, route(async (req, res) => {
    ...
    const [remote, channels, roles, changes, events, preferences, draft] = await Promise.all([
    ...
    res.json({ guild: { ...guild, name: remote.ok ? remote.data.name : guild.name }, connection: { status, checked_at: new Date().toISOString(), readable },
      ...
      changeSets: changes.rows, activity: events.rows, draft: draft.rows[0] || null,
  ...
  app.post('/api/change-sets', requireUser, route(async (req, res) => {
    const guild = await guildFor(req);
    await requirePlanCapacity(req.user, 'changeSetsPerMonth');
    const current = await snapshot(guild.id);
    let plan;
    if (Array.isArray(req.body.operations)) plan = { template_key: 'custom', name: 'تعديلات القنوات والرتب', operations: normalizeOperations(req.body.operations, current) };
    else {
  ...
  app.post('/api/change-sets/:id/apply', requireUser, route(async (req, res) => {
  ...
  app.put('/api/workspace/:guildId/preferences', requireUser, route(async (req, res) => {
    const guild = await guildFor(req);
  ...
  app.get('/api/workspace/:guildId/analytics', requireUser, route(async (req, res) => {
    const guild = await guildFor(req); const days = [7, 14, 30].includes(Number(req.query.days)) ? Number(req.query.days) : 7;
  ...
  app.post('/api/workspace/:guildId/schedules', requireUser, route(async (req, res) => {
    ...
    const result = await pool.query('INSERT INTO scheduled_messages(user_id,guild_id,channel_id,content,run_at,repeat,timezone) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING *', [req.user.id, guild.id, schedule.channel_id, schedule.content, schedule.run_at, schedule.repeat, schedule.timezone]);
    await audit(req.user.id, 'schedule.create', 'guild', guild.id, { schedule_id: result.rows[0].id });
    res.status(201).json({ schedule: result.rows[0] });
  }));
  app.post('/api/workspace/:guildId/schedules/:id/cancel', requireUser, route(async (req, res) => {
    const guild = await guildFor(req);
...
export function startScheduleRunner({ pool, discordBotFetch, authorizedGuild }) {
  ...
  const tick = async () => {
    ...
    try {
      ...
      await pool.query("DELETE FROM community_activity WHERE day < CURRENT_DATE - 29");
