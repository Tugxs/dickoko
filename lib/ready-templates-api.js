...
export async function migrateReadyTemplates(pool) {
  ...
  await pool.query("ALTER TABLE ready_template_runs ADD COLUMN IF NOT EXISTS usage_units INT NOT NULL DEFAULT 1; ALTER TABLE ready_template_runs ADD COLUMN IF NOT EXISTS executor TEXT NOT NULL DEFAULT 'diskoko'");
...
export function mountReadyTemplates(app, { pool, requireUser, requireWriteAccess, authorizedGuild, discordBotFetch: discord, audit, requirePlanCapacity, botStatus }) {
  ...
  app.post('/api/workspace/:guildId/ready-templates/review', requireUser, requireWriteAccess, route(async (req, res) => {
    ...
    const review = { ...diff, fingerprint: fingerprint(current), guildName: current.guild.name, usageUnits, counts: { channels: current.channels.length, roles: current.roles.length } };
    ...
    try {
      ...
      if (capacity.used + usageUnits > capacity.limit) throw problem(`القالب يحتاج ${usageUnits} متغيرًا، والمتبقي من باقتك ${Math.max(0, capacity.limit - capacity.used)}. عدّل القالب أو الباقة قبل التنفيذ.`, 402);
  ...
  app.post('/api/workspace/:guildId/ready-templates/runs/:id/apply', requireUser, requireWriteAccess, route(async (req, res) => {
    ...
    try {
      ...
      const required = P.ManageChannels | P.ManageRoles | ((run.definition.features.ticket.enabled || run.definition.features.logs.enabled) ? P.ViewChannel | P.SendMessages | P.ReadMessageHistory : 0n) | (run.definition.features.ticket.enabled ? P.EmbedLinks : 0n);
      ...
      if (run.status === 'draft') {
        ...
        try {
          ...
          if (capacity.used + run.usage_units > capacity.limit) throw problem('لم تعد متغيرات الباقة المتبقية تكفي هذا القالب. راجع الاستخدام أو الباقة.', 402);
        ...
        try {
          ...
          if (step.kind === 'role') {
            ...
            if (existing) resourceId = existing.id;
              ...
              const body = { name, type: channel.type, parent_id: parentId, permission_overwrites: everyoneOverwrite(guild.id, channel.access, [roleId, ...privileged, botRole?.id].filter(Boolean)) };
            ...
            const routes = logs.mode === 'routed' ? logs.routes.map(rule => ({ key: rule.key, sourceIds: rule.sourceKeys.map(key => map.get(`channel:${key}`)), targetId: map.get(`channel:${rule.targetKey}`) })) : [];
