...
export async function repairLegacyTicketControls(bot, pool) {
  const tickets = (await pool.query("SELECT panel_id,channel_id FROM diskoko_tickets WHERE status='open' ORDER BY created_at DESC LIMIT 100")).rows;
  for (const ticket of tickets) {
    try {
      ...
      for (const message of messages) {
        if (message.author?.id !== bot.user.id || !message.components?.some(row => row.components?.some(component => component.customId === `diskoko:claim:${ticket.panel_id}`))) continue;
        await message.edit({ components: ticketOwnerControls(ticket.panel_id) });
      }
...
export async function handleInteractiveButton(interaction, pool) {
  ...
  if (kind === 'ticket') {
    ...
    try {
      ...
      await channel.send({ content: `🎫 تذكرة خاصة بـ <@${interaction.user.id}>. اكتب طلبك هنا، وسيطلع عليه فريق الدعم.\nيمكن لفريق الدعم استلام التذكرة عبر الأمر /diskoko claim داخل هذه القناة.`, components: ticketOwnerControls(id), allowedMentions: { users: [interaction.user.id] } });
  ...
  if (kind === 'reopen') {
    ...
    await interaction.channel.send({ content: 'أُعيد فتح التذكرة. يستطيع فريق الدعم استلامها عبر /diskoko claim.', components: ticketOwnerControls(id), allowedMentions: { parse: [] } });
