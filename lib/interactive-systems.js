  ...
  app.post('/api/ai/requests/:id/launch-interactive', requireUser, requireWriteAccess, async (req, res, next) => {
    ...
    try {
      ...
      if (plan.kind === 'welcome') {
        ...
        if (composite && req.body.media) return reject(400, 'دمج صورة العضو داخل GIF غير متاح. استخدم GIF متحركًا مع صورة العضو في بطاقة Discord.');
        const banner = composite ? validatedWelcomeBackground(req.body.image) : validatedAiMedia(req.body.media) || validatedAiImage(req.body.image) || item.attachment;
