    ...
    try {
      ...
      if (plan.kind === 'rules') {
        ...
        for (const [index, page] of pages.entries()) {
          const sent = await discordBotFetch(`/channels/${channel.id}/messages`, discordMessageOptions(page, index === 0 ? image : null, req.body.imagePosition));
          if (!sent.ok || !sent.data?.id) throw Object.assign(new Error('تعذر نشر كل بطاقات القوانين. تحقق من صلاحيات البوت في القناة، ثم أعد المحاولة.'), { status: 502 });
        ...
        await client.query('COMMIT'); rulesCommitted = true; return res.json({ ok: true, messageId: sentMessageId, messageIds: sentRuleMessages, channelId: channel.id });
      }
      if (plan.kind === 'event') {
      ...
      if (!rulesCommitted && ruleChannelId) for (const messageId of sentRuleMessages.reverse()) await discordBotFetch(`/channels/${ruleChannelId}/messages/${messageId}`, { method: 'DELETE' }).catch(() => {});
