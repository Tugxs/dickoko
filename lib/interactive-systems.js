...
export function validateInteractiveDraft(plan, body = {}) {
  if (!['poll', 'event', 'welcome', 'giveaway', 'tickets'].includes(plan?.kind)) return 'نوع الخطة غير مدعوم.';
  ...
  app.post('/api/ai/requests/:id/launch-interactive', requireUser, requireWriteAccess, async (req, res, next) => {
    ...
    try {
      ...
      if (plan.kind === 'welcome' && req.body.composite === true) {
        ...
        if (req.body.media?.mime === 'image/gif') await validateWelcomeGif(Buffer.from(req.body.media.base64, 'base64'));
      }
      ...
      if (plan.kind === 'welcome') {
        ...
        if (composite && req.body.media) {
          ...
          try { await validateWelcomeGif(Buffer.from(media.base64, 'base64')); }
