  ...
  app.post('/api/ai/requests/:id/launch-interactive', requireUser, requireWriteAccess, async (req, res, next) => {
    ...
    try {
      ...
      if (plan.kind === 'welcome') {
        ...
        const avatarPosition = ['left', 'right', 'top'].includes(req.body.avatarPosition) ? req.body.avatarPosition : 'right';
        const bannerPosition = req.body.bannerPosition === 'above' ? 'above' : 'below';
        await client.query(`INSERT INTO diskoko_welcome_cards(guild_id,request_id,channel_id,title,description,color,banner,avatar_position,banner_position) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9)
          ON CONFLICT(guild_id) DO UPDATE SET request_id=EXCLUDED.request_id,channel_id=EXCLUDED.channel_id,title=EXCLUDED.title,description=EXCLUDED.description,color=EXCLUDED.color,banner=EXCLUDED.banner,avatar_position=EXCLUDED.avatar_position,banner_position=EXCLUDED.banner_position,updated_at=NOW()`, [item.guild_id, item.id, channel.id, title, description, cardColor(req.body.color), banner ? JSON.stringify(banner) : null, avatarPosition, bannerPosition]);
        await client.query("UPDATE ai_requests SET interactive_channel_id=$1,interactive_kind='welcome',published_at=NOW(),proposal=$3 WHERE id=$2", [channel.id, item.id, { ...item.proposal, interactive: { ...plan, title, description, color: req.body.color, avatarPosition, bannerPosition } }]);
...
export async function handleInteractiveButton(interaction, pool) {
  ...
  if (kind === 'giveaway') {
    ...
    if (result.rowCount && giveaway.channel_id === interaction.channelId && giveaway.message_id === interaction.message?.id) {
      ...
      if (embeds.length) await interaction.message.edit({ embeds }).catch(error => console.error('Giveaway participant count update failed:', error.message));
    }
...
export async function sendWelcomeCard(member, pool) {
  ...
  const card = (await pool.query('SELECT channel_id,title,description,color,banner,avatar_position,banner_position FROM diskoko_welcome_cards WHERE guild_id=$1', [member.guild.id])).rows[0];
