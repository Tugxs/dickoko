...
export function discordMessageOptions(payload, attachment, imagePosition = 'above') {
  ...
  const embeds = imagePosition === 'below' ? [...(payload.embeds || []), imageEmbed] : [imageEmbed, ...(payload.embeds || [])];
...
export function mountInteractiveSystems(app, { pool, requireUser, requireWriteAccess, authorizedGuild, discordBotFetch, requirePlanCapacity }) {
  app.post('/api/ai/requests/:id/launch-interactive', requireUser, requireWriteAccess, async (req, res, next) => {
    ...
    try {
      ...
      const image = validatedAiImage(req.body.image) || item.attachment;
      ...
      if (plan.kind === 'poll') {
        ...
        if (incompleteLibraryValue(question) || !Array.isArray(options) || options.length < 2 || options.length > 5 || options.some(incompleteLibraryValue) || new Set(options.map(value => value.toLocaleLowerCase('ar'))).size !== options.length) return reject(400, 'اكتب سؤالًا وخيارين إلى خمسة خيارات مختلفة');
      ...
      if (plan.kind === 'giveaway') {
        ...
        if (incompleteLibraryValue(prize) || !Number.isInteger(durationMinutes) || durationMinutes < 5 || durationMinutes > 43200 || !Number.isInteger(winnerCount) || winnerCount < 1 || winnerCount > 20) return reject(400, 'تحقق من الجائزة والمدة وعدد الفائزين');
