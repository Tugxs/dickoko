...
export function registerGuildActivityLogs(client, pool, onlyGuildId = null) {
  ...
  async function capture({ guildId, sourceId, event, actorId, messageId, commandName }) {
    ...
    try {
      ...
      for (const targetId of activityLogDestinations(config, event, sourceId)) {
        ...
        const line = `• ${EVENT_LABELS[event]}${command} في <#${sourceId}>${actorId ? ` بواسطة <@${actorId}>` : ''}${link}`;
