...
export const supportedCommands = ['help', 'ping', 'about'];
...
export function normalizeOperations(input, snapshot) {
  ...
  return input.map((item, index) => {
    ...
    if (!name || name.length > 100) throw problem('اسم العنصر مطلوب ويجب ألا يتجاوز 100 حرف.');
    ...
    if (op.resource_type === 'channel') {
      if (action === 'create') {
        ...
        if (![0, 2].includes(op.type)) throw problem('اختر قناة نصية أو صوتية.');
      }
