...
export async function connectedBotMetadata(pool, guildId) {
  const row = (await pool.query('SELECT bot_user_id,bot_name,updated_at,retry_at,member_joins FROM ai_bot_connections WHERE guild_id=$1', [guildId])).rows[0];
  const client = clients.get(guildId);
  return row ? { id: row.bot_user_id, name: row.bot_name, updatedAt: row.updated_at, online: Boolean(client?.isReady() && client.guilds.cache.has(guildId)), memberJoins: client?._diskokoMemberJoins ?? row.member_joins ?? false, retryAt: row.retry_at } : null;
}
...
function scheduleAiBotRestore(pool, row, delayMs) {
  ...
  const timer = setTimeout(() => { reconnectTimers.delete(guildId); void (async () => {
    try {
      const current = (await pool.query('SELECT guild_id,bot_user_id,token_encrypted,member_joins FROM ai_bot_connections WHERE guild_id=$1', [guildId])).rows[0];
      if (!current) return;
...
export function mountAiBotConnections(app, { pool, requireUser, requireWriteAccess, authorizedGuild, requirePlanCapacity, audit }) {
  ...
  app.post('/api/ai/bot-connection', requireUser, requireWriteAccess, async (req, res, next) => { try {
    ...
    if (retryAt) { await stopAiBot(guild.id); scheduleAiBotRestore(pool, { guild_id: guild.id }, retryAt.getTime() - Date.now()); }
    await audit(req.user.id, 'ai.bot.connect', 'guild', guild.id, { botId: identity.data.id });
    res.status(retryAt ? 202 : 200).json({ bot: await connectedBotMetadata(pool, guild.id), pending: Boolean(retryAt) });
  } catch (error) { next(error); } });
