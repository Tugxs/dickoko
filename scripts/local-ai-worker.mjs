...
async function respond(job) {
  ...
  const proposal = normalizeAiProposal(await propose(job, context, guild, answer));
...
async function propose(job, context, guild, answer) {
  ...
  const instructions = [
    ...
    'إذا كان طلب العميل جيف آواي أو تذاكر أو استطلاعًا، لا تضف معه عمليات إنشاء تصنيف أو قناة أو رتبة لم يطلبها صراحة. اختيار قناة النشر الموجودة يتم في بطاقة المراجعة. إنشاء قناة جديدة للدعم يظهر كخيار واضح في بطاقة المراجعة، وليس تخمينًا من النموذج.',
    ...
    'لبطاقة تحميل ملف استخدم interactive: {"kind":"download","title":"عنوان البطاقة","description":"وصف الملف","channel":"قناة النشر"}. سيختار العميل الملف عند المراجعة، ثم ينشر ديسكوكو بطاقة بزر تنزيل مرتبط بالملف. لا تجعلها رسالة نصية عادية ولا تدّع رفع الملف قبل تأكيد العميل.',
