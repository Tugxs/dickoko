  ...
  const renderMessages = () => {
    ...
    const openSpecialReview = (item, plan) => {
      ...
      const update = () => {
        ...
        const animatedWelcome = plan.kind === 'welcome' && $('#aiSpecialImage').files[0]?.type === 'image/gif';
        if (plan.kind === 'welcome') {
          $('#aiWelcomeComposite').disabled = animatedWelcome;
          if (animatedWelcome) $('#aiWelcomeComposite').checked = false;
        }
