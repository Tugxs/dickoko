...
async function assistant() {
  ...
  const renderMessages = () => {
    ...
    const openSpecialReview = (item, plan) => {
      ...
      const rulesFields = `<label>عنوان القوانين<input id="aiSpecialTitle" maxlength="180" value="${esc(plan.title || '📜 قوانين السيرفر')}"></label><label>مقدمة قصيرة (اختياري)<textarea id="aiSpecialDescription" maxlength="500" rows="2">${esc(plan.description || '')}</textarea></label><label>طريقة عرض القوانين<select id="aiRulesStyle"><option value="single" ${plan.style === 'single' ? 'selected' : ''}>بطاقة واحدة مرقمة</option><option value="sections" ${plan.style === 'sections' ? 'selected' : ''}>أقسام داخل بطاقة واحدة</option><option value="cards" ${plan.style === 'cards' ? 'selected' : ''}>بطاقة مستقلة لكل قانون</option></select></label><label>القوانين — كل قانون في سطر<textarea id="aiRulesList" rows="9" placeholder="اكتب كل قانون في سطر مستقل">${esc((plan.rules || []).join('\n'))}</textarea></label>${format}<small class="form-note">من قانونين إلى ٨ قوانين. الحد الأقصى لكل قانون ٢٥٠ حرفًا، ويمكنك تعديل الصياغة والإيموجيات.</small><label>صورة أو GIF للقوانين (اختياري)<input id="aiSpecialImage" type="file" accept="image/png,image/jpeg,image/webp,image/gif"></label>${imageStyleFields('aiSpecialImage')}<label>موضع الصورة العادية<select id="aiRulesImagePosition"><option value="above">فوق بطاقة القوانين</option><option value="below">تحت بطاقة القوانين</option></select></label>`;
      ...
      const update = () => {
        ...
        const ruleLines = plan.kind === 'rules' ? $('#aiRulesList').value.split(/\r?\n/).map(line => line.trim()).filter(Boolean) : [];
        const ruleStyle = plan.kind === 'rules' ? $('#aiRulesStyle').value : '';
        const text = plan.kind === 'poll' ? `${$('#aiSpecialDescription').value}\nاختر إجابة واحدة. يمكنك تغيير صوتك.` : plan.kind === 'rules' && ruleStyle === 'single' ? [$('#aiSpecialDescription').value, ...ruleLines.map((rule, index) => `**${index + 1}.** ${rule}`), 'يرجى قراءة القوانين والالتزام بها.'].filter(Boolean).join('\n\n') : plan.kind === 'rules' ? $('#aiSpecialDescription').value || 'يرجى قراءة القوانين والالتزام بها.' : $('#aiSpecialDescription').value.replaceAll('{member}', '@عضو جديد').replaceAll('{name}', 'عضو جديد');
        $('#aiSpecialPreviewBody').innerHTML = discordMarkdownPreview(text);
        const previewOptions = $('#aiSpecialPreviewOptions');
        previewOptions.innerHTML = plan.kind === 'poll' ? optionFiles.map((entry, index) => `<div class="ai-poll-preview-option">${entry.file ? `<img src="${fileUrl(entry.file)}" alt="صورة الخيار ${index + 1}">` : ''}<span>${index + 1}. ${emojiPreviewText(entry.text || 'الخيار')}</span></div>`).join('') : plan.kind === 'rules' && ruleStyle !== 'single' ? ruleLines.map((rule, index) => `<div class="${ruleStyle === 'cards' ? 'ai-rules-separate-card' : 'ai-rules-field'}"><b>القانون ${index + 1}</b><p>${discordMarkdownPreview(rule)}</p></div>`).join('') : '';
      ...
      $('#dialogContent').querySelectorAll('[data-special-format]').forEach(button => button.onclick = () => {
        const field = plan.kind === 'rules' ? $('#aiRulesList') : $('#aiSpecialDescription'), start = field.selectionStart, end = field.selectionEnd, selected = field.value.slice(start, end) || 'النص';
    ...
    thread.querySelectorAll('[data-ai-interactive]').forEach(button => button.onclick = () => {
      ...
      const plan = item?.proposal?.interactive; if (!plan || !['giveaway', 'tickets', 'poll', 'event', 'welcome', 'scheduled_event', 'rules'].includes(plan.kind)) return;
