  ...
  const renderMessages = () => {
    ...
    const openSpecialReview = (item, plan) => {
      ...
      const update = () => {
        ...
        $('#aiSpecialPreviewOptions').innerHTML = plan.kind === 'poll' ? optionFiles.map((entry, index) => `<div class="ai-poll-preview-option">${entry.file ? `<img src="${fileUrl(entry.file)}" alt="صورة الخيار ${index + 1}">` : ''}<span>${index + 1}. ${esc(entry.text || 'الخيار')}</span></div>`).join('') : plan.kind === 'welcome' ? '<small>◉ صورة العضو الجديد تظهر هنا عند انضمامه</small>' : '';
