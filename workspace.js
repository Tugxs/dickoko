...
function alertRows(items) {
  return `<div class="rows">${items.map(item => `<div class="row"><span class="row-icon">⚠</span><div class="row-main"><b>${esc(item.title)}</b><small>${esc(item.detail)}</small></div><div class="actions">${item.target ? action(item.target === 'automation' ? 'عرض الرسائل' : 'افتح الإعدادات', item.target, 'secondary') : ''}${item.channelId ? `<a class="btn secondary" href="https://discord.com/channels/${encodeURIComponent(state.guild)}/${encodeURIComponent(item.channelId)}" target="_blank" rel="noopener">افتح القناة ↗</a>` : ''}${item.kind === 'giveaway' && item.retryable ? `<button class="btn secondary" data-alert-retry="${esc(item.id)}">محاولة واحدة</button>` : ''}${item.kind === 'schedule' ? `<button class="btn secondary" data-alert-cancel="${esc(item.id)}">إلغاء المهمة</button>` : ''}${item.id ? `<button class="btn text" data-alert-dismiss="${esc(item.kind)}:${esc(item.id)}">إخفاء التنبيه</button>` : `<button class="btn text" data-alert-refresh>إعادة التحقق</button>`}</div></div>`).join('')}</div>`;
}
