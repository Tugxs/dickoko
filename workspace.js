...
function renderReadyEditor() {
  ...
  if (botNotice) {
    botNotice.classList.add('ready-executor-notice');
    botNotice.innerHTML = `<div class="ready-bot-choice"><div class="ready-bot-choice-head"><b>البوت الذي سينفذ القالب</b><small>اختر البوت المناسب قبل معاينة التغييرات.</small></div><div class="ready-bot-options"><label class="ready-bot-option"><input type="radio" name="readyExecutor" value="diskoko" ${state.readyExecutor !== 'custom' ? 'checked' : ''}><span><strong>بوت ديسكوكو</strong><small>${state.data.bot?.online ? '● متصل وجاهز' : 'حالة الاتصال تُفحص قبل التنفيذ'}</small></span></label><label class="ready-bot-option"><input type="radio" name="readyExecutor" value="custom" ${state.readyExecutor === 'custom' ? 'checked' : ''}><span><strong>بوتك الخاص</strong><small>${state.readyCustomBot ? `${esc(state.readyCustomBot.name)} · ${state.readyCustomBot.online ? 'متصل' : 'غير متصل'}` : 'لم يُربط بعد'}</small></span></label></div><p>سنفحص اتصال البوت وصلاحياته قبل أي تغيير في سيرفرك.</p></div><div class="ready-executor-action">${action(state.readyCustomBot ? 'إدارة ربط البوت' : 'ربط بوتك الخاص', 'settings')}</div>`;
  }
