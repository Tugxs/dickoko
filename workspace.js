...
async function assistant() {
  ...
  const renderMessages = () => {
    ...
    thread.querySelectorAll('[data-ai-structure]').forEach(button => button.onclick = () => {
      ...
      const example = /هيكل|قسم ترحيب/.test(item.library_title || '') ? 'تصنيف: البداية\nقناة: ابدأ-هنا\nقناة: القوانين' : /صوتية/.test(item.library_title || '') ? 'صوتية: المجلس' : /رتبة/.test(item.library_title || '') ? 'رتبة: عضو' : /تصنيف/.test(item.library_title || '') ? 'تصنيف: المجتمع' : 'قناة: اسم-القناة';
      modal('تجهيز هيكل السيرفر', `<p class="form-note">اكتب كل عنصر في سطر مستقل. الأنواع: تصنيف، قناة، صوتية، رتبة. راجع الأسماء قبل التطبيق؛ العناصر الموجودة لن تتكرر إذا طابقت الخطة.</p><label>العناصر المطلوب إنشاؤها<textarea id="aiStructureLines" rows="7" maxlength="1600" placeholder="${esc(example)}"></textarea></label><section class="ai-discord-preview" aria-label="معاينة هيكل السيرفر"><div class="ai-discord-preview-head"><b>معاينة داخل ${esc(state.data?.guild?.name || 'سيرفرك')}</b><small>العناصر الجديدة مميزة باللون البنفسجي</small></div><div class="ai-discord-server"><aside class="ai-discord-server-channels"><b>${esc(state.data?.guild?.name || 'السيرفر')}</b>${(state.data?.channels || []).filter(channel => [0, 2, 4].includes(channel.type)).slice(0, 8).map(channel => `<span>${channel.type === 4 ? '▾' : channel.type === 2 ? '◖' : '#'} ${esc(channel.name)}</span>`).join('')}<div id="aiStructurePreviewItems"></div></aside><div class="ai-discord-server-chat"><div class="ai-discord-channel-name">معاينة ترتيب السيرفر</div><p class="ai-structure-preview-hint">هذه معاينة للهيكل فقط. سيعرض البوت التغييرات الدقيقة والصلاحيات في خطة المراجعة التالية قبل التطبيق.</p></div></div></section><p class="form-note">هذه مسودة فقط. ستظهر خطة تغييرات منفصلة للمراجعة قبل أن يغيّر البوت السيرفر.</p>`, '<button id="aiStructureCancel" class="btn secondary">إلغاء</button><button id="aiStructureReview" class="btn primary">مراجعة خطة التغييرات</button>');
      ...
      $('#aiStructureReview').onclick = async event => { event.currentTarget.disabled = true; try {
        ...
        const operations = lines.map(line => {
          ...
          return { resource_type: match[1] === 'تصنيف' ? 'category' : match[1] === 'رتبة' ? 'role' : 'channel', action: 'create', name, ...(match[1] === 'قناة' || match[1] === 'صوتية' ? { type: match[1] === 'صوتية' ? 2 : 0 } : {}) };
        });
    ...
    thread.querySelectorAll('[data-ai-message]').forEach(button => button.onclick = () => {
      ...
      $('#aiMessageImage').onchange = event => {
        ...
        if (!banner) { banner = document.createElement('img'); banner.className = 'ai-discord-banner'; banner.alt = 'معاينة الصورة'; $('.ai-message-preview-body').append(banner); }
