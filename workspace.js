...
async function assistant() {
  ...
  $('#workspace .ai-chat-layout').insertAdjacentHTML('beforeend', `<aside class="panel ai-library"><div class="panel-head"><div><h3>مكتبة AI ديسكوكو</h3><small>${fmt(aiPromptLibrary.length)} مهمة جاهزة للتخصيص</small></div></div><div class="ai-library-controls"><label for="aiLibrarySearch" class="sr-only">ابحث في الاقتراحات</label><input id="aiLibrarySearch" type="search" placeholder="ابحث عن مهمة…"><div id="aiLibraryCategories" class="ai-library-categories"></div></div><div id="aiLibraryList" class="ai-library-list"></div><p class="ai-library-note">اختر فكرة أو خطوة من الناتج، ثم راجع الإجراء الفعلي قبل تطبيقه. سيبيّن لك المساعد ما لا يستطيع البوت تنفيذه حاليًا.</p></aside>`);
  ...
  const renderMessages = () => {
    ...
    thread.querySelectorAll('[data-ai-message]').forEach(button => button.onclick = () => {
      ...
      $('#aiMessageSend').onclick = async event => { event.currentTarget.disabled = true; try {
        ...
        await api(`/api/ai/requests/${encodeURIComponent(item.id)}/send-message`, { method: 'POST', body: JSON.stringify({ confirmed: true, channelId: $('#aiMessageChannel').value, content: $('#aiMessageContent').value, image, imagePosition: $('#aiMessageImagePosition').value }) });
        closeDialog(); await loadMessages(); toast('نُشرت الرسالة في Discord.');
    ...
    thread.querySelectorAll('[data-ai-interactive]').forEach(button => button.onclick = () => {
      ...
      modal(plan.kind === 'giveaway' ? 'مراجعة الجيف آواي' : plan.kind === 'poll' ? 'مراجعة الاستطلاع' : plan.kind === 'download' ? 'مراجعة بطاقة تحميل الملف' : 'مراجعة لوحة تذاكر الدعم', `${fields}<section class="ai-discord-preview" aria-label="معاينة قبل النشر"><div class="ai-discord-preview-head"><b>معاينة في Discord</b><small>تتحدث مع تعديل الحقول · لا تنشر شيئًا</small></div>${item.has_attachment && plan.kind !== 'poll' ? `<img class="ai-discord-banner" src="/api/ai/requests/${encodeURIComponent(item.id)}/attachment" alt="البنر المرفق">` : ''}<div class="ai-discord-embed"><b id="aiPreviewTitle"></b><p id="aiPreviewDescription"></p><small id="aiPreviewMeta"></small></div><span class="ai-discord-button" id="aiPreviewButton"></span></section><label class="check-row"><input id="aiInteractiveConfirmed" type="checkbox">راجعت الإعدادات وأوافق على النشر في Discord.</label>`, '<button class="btn secondary" id="aiInteractiveCancel">إلغاء</button><button class="btn primary" id="aiInteractiveLaunch" disabled>نعم، أؤكد التنفيذ</button>');
      if (item.has_attachment && ['tickets', 'giveaway'].includes(plan.kind)) {
        $('.ai-discord-preview').insertAdjacentHTML('beforebegin', '<label>موضع الصورة في Discord<select id="aiInteractiveImagePosition"><option value="above">فوق التفاصيل</option><option value="below">تحت التفاصيل</option></select></label>');
        $('#aiInteractiveImagePosition').onchange = event => {
      ...
      $('#aiInteractiveLaunch').onclick = async event => { event.currentTarget.disabled = true; try {
        if (plan.kind === 'download') {
          ...
          await api(`/api/ai/requests/${encodeURIComponent(item.id)}/launch-download`, { method: 'POST', body: JSON.stringify({ confirmed: true, channelId: $('#aiInteractiveChannel').value, title: $('#aiDownloadTitle').value, description: $('#aiDownloadDescription').value, file: await prepareDownloadFile(file) }) });
  ...
  $('#assistantForm').onsubmit = run(async event => {
    ...
    try {
      ...
      const result = await api('/api/ai/requests', { method: 'POST', body: JSON.stringify({ guildId: guild, conversationId: selected || undefined, prompt, image, libraryMode: selectedTemplate?.mode, libraryTitle: selectedTemplate?.title }) });
      if (!active()) return;
