...
function renderReadyEditor() {
  ...
  if (botNotice) botNotice.innerHTML = `<div class="ready-bot-choice"><b>البوت الذي سينفذ القالب</b><label><input type="radio" name="readyExecutor" value="diskoko" ${state.readyExecutor !== 'custom' ? 'checked' : ''}>بوت ديسكوكو ${state.data.bot?.online ? '· متصل' : '· تحقق من الاتصال'}</label><label><input type="radio" name="readyExecutor" value="custom" ${state.readyExecutor === 'custom' ? 'checked' : ''}>بوتك الخاص ${state.readyCustomBot ? `· ${esc(state.readyCustomBot.name)}${state.readyCustomBot.online ? ' متصل' : ' غير متصل'}` : '· لم يُربط بعد'}</label><small>يستخدم التنفيذ البوت الذي تختاره هنا، وسيُفحص اتصاله وصلاحياته قبل أي تغيير.</small></div>${action(state.readyCustomBot ? 'إدارة ربط البوت' : 'ربط بوتك الخاص', 'settings')}`;
  ...
  const [welcomeSection, ticketSection] = document.querySelectorAll('.ready-feature');
