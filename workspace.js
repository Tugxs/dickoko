...
async function assistant() {
  ...
  const renderProposal = item => {
    ...
    return `<div class="ai-action-card"><span class="ai-action-step">الخطوة الأخيرة قبل التنفيذ</span><b>راجع ما سيتغير في ${esc(state.data?.guild?.name || 'سيرفرك')}</b>${operations.length ? `<p>${operations.map(op => `${op.resource_type === 'role' ? 'رتبة' : op.resource_type === 'category' ? 'تصنيف' : 'قناة'}: ${esc(op.name)}`).join(' · ')}</p><button class="btn primary small" type="button" data-ai-plan="${esc(item.id)}">مراجعة التغييرات وتأكيدها</button>` : ''}${proposal.message ? `<p>رسالة إلى #${esc(proposal.message.channel)}: ${esc(proposal.message.content)}</p>${item.sent_message_id ? `<a class="btn small secondary" href="https://discord.com/channels/${encodeURIComponent(guild)}/${encodeURIComponent(item.sent_channel_id)}/${encodeURIComponent(item.sent_message_id)}" target="_blank" rel="noopener noreferrer">تم الإرسال · عرض في Discord</a>` : `<button class="btn primary small" type="button" data-ai-message="${esc(item.id)}">مراجعة الرسالة وتأكيد النشر</button>`}` : ''}${proposal.interactive ? `<p>${proposal.interactive.kind === 'giveaway' ? `🎉 جيف آواي: ${esc(proposal.interactive.prize)} · ${esc(proposal.interactive.winnerCount)} فائز · ${esc(proposal.interactive.durationMinutes)} دقيقة` : `🎫 لوحة تذاكر: ${esc(proposal.interactive.title)}`}</p>${item.interactive_message_id ? `<a class="btn small secondary" href="https://discord.com/channels/${encodeURIComponent(guild)}/${encodeURIComponent(item.interactive_channel_id)}/${encodeURIComponent(item.interactive_message_id)}" target="_blank" rel="noopener noreferrer">تم النشر · عرض في Discord</a>` : `<button class="btn primary small" type="button" data-ai-interactive="${esc(item.id)}">مراجعة النظام التفاعلي وتشغيله</button>`}` : ''}<small>لم يُنفّذ شيء بعد. يمكنك مراجعة التفاصيل وتعديل الرسالة قبل التأكيد.</small></div>`;
  };
  const renderList = () => {
  ...
  const renderMessages = () => {
    ...
    thread.innerHTML = messages.length ? messages.map(item => `<div class="ai-turn"><div class="ai-bubble user"><b>أنت</b><div>${esc(item.prompt)}</div>${item.has_attachment ? `<img class="ai-sent-image" src="/api/ai/requests/${encodeURIComponent(item.id)}/attachment" alt="الصورة المرفقة بهذه الرسالة">` : ''}</div><div class="ai-bubble assistant"><b>AI ديسكوكو</b><div>${esc(item.status === 'completed' ? item.answer || '' : item.status === 'failed' ? item.error || 'تعذر توليد الرد. حاول مجددًا.' : 'جارٍ تجهيز الرد…')}</div></div>${renderProposal(item)}</div>`).join('') : `<div class="ai-welcome"><span>✦</span><h2>أهلًا، أنا AI ديسكوكو</h2><p>قل لي وش تحتاج في سيرفرك، وبساعدك بخطوات واضحة. تقدر تتابع معي في نفس المحادثة، وسأفهم سياق كلامنا.</p><div class="ai-suggestions"><button type="button" data-ai-suggestion="اقترح لي ترتيب رومات لسيرفر ألعاب">رتب لي الرومات</button><button type="button" data-ai-suggestion="اقترح رتب وصلاحيات مناسبة لمجتمعي">نظّم الرتب</button></div></div>`;
    ...
    thread.querySelectorAll('[data-ai-interactive]').forEach(button => button.onclick = () => {
      ...
      const channelField = `<label>قناة النشر<select id="aiInteractiveChannel"><option value="">اختر قناة نصية</option>${channels.map(channel => `<option value="${esc(channel.id)}" ${channel.name.toLowerCase() === plan.channel.toLowerCase() ? 'selected' : ''}>#${esc(channel.name)}</option>`).join('')}</select></label>`;
      const fields = plan.kind === 'giveaway' ? `${channelField}<label>الجائزة<input id="aiPrize" maxlength="160" value="${esc(plan.prize)}"></label><div class="form-grid two"><label>المدة بالدقائق<input id="aiDuration" type="number" min="5" max="43200" value="${Number(plan.durationMinutes)}"></label><label>عدد الفائزين<input id="aiWinners" type="number" min="1" max="20" value="${Number(plan.winnerCount)}"></label></div><p class="form-note">ينشر البوت زر مشاركة ويسحب الفائزين عشوائيًا عند انتهاء المدة.</p>` : `${channelField}<label>عنوان لوحة الدعم<input id="aiTicketTitle" maxlength="100" value="${esc(plan.title)}"></label><label>الوصف<textarea id="aiTicketDescription" maxlength="800" rows="3">${esc(plan.description)}</textarea></label><label>تصنيف التذاكر (اختياري)<select id="aiTicketCategory"><option value="">دون تصنيف</option>${(state.data.channels || []).filter(channel => channel.type === 4).map(channel => `<option value="${esc(channel.id)}">${esc(channel.name)}</option>`).join('')}</select></label><label>رتبة فريق الدعم (مطلوبة)<select id="aiStaffRole"><option value="">اختر رتبة الدعم</option>${(state.data.roles || []).filter(role => role.id !== guild).map(role => `<option value="${esc(role.id)}">${esc(role.name)}</option>`).join('')}</select></label><p class="form-note">يفتح زر الدعم قناة خاصة لكل عضو. التذكرة خاصة بصاحبها ورتبة الدعم المحددة.</p>`;
      modal(plan.kind === 'giveaway' ? 'مراجعة الجيف آواي' : 'مراجعة لوحة تذاكر الدعم', `${fields}<label class="check-row"><input id="aiInteractiveConfirmed" type="checkbox">راجعت الإعدادات وأوافق على النشر في Discord.</label>`, '<button class="btn secondary" id="aiInteractiveCancel">إلغاء</button><button class="btn primary" id="aiInteractiveLaunch" disabled>نشر وتشغيل</button>');
      ...
      $('#aiInteractiveLaunch').onclick = async event => { event.currentTarget.disabled = true; try {
        const body = { confirmed: true, channelId: $('#aiInteractiveChannel').value, ...(plan.kind === 'giveaway' ? { prize: $('#aiPrize').value, durationMinutes: Number($('#aiDuration').value), winnerCount: Number($('#aiWinners').value) } : { title: $('#aiTicketTitle').value, description: $('#aiTicketDescription').value, categoryId: $('#aiTicketCategory').value, staffRoleId: $('#aiStaffRole').value }) };
